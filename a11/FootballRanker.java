...
public class FootballRanker {
    ...
    private double printRootMeanSquareError(List<String> humanRanks,
        ...
        double result = 0;
        int count = 0;
        //iterates through the AP ranks and determines the root mean square error
        //between the AP ranks and calculated ranks
        for(String team : humanRanks){
            count++;
            int predicted = 1;
            Iterator<AllPathsInfo> it = paths.iterator();
            //increments predicted
            while(it.hasNext() && !it.next().getName().equals(team)){
                predicted++;
            }
            int diff = actual - predicted;
            result = result + Math.pow(diff, 2);
            if(showResults){
                System.out.println(String.format("%1$-" + PADDING  + "s", team)
                        + " - actual rank: " + (count) + " predicted rank: "
                        + (predicted));
            }
            actual++;
        }
