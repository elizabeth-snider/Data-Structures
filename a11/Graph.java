...
public class Graph {
    ...
    public void dijkstra(String startName) {
        ...
        vertices.get(currentStartVertexName).numEdgesFromStartVertex = 0;
        //setting the cost of the start vertex to 0
        //and all other vertices to infinity
        Path path = new Path();
        for(Vertex v : vertices.values()){
            if(v.equals(start)){
                v.weightedCostFromStartVertex = 0;
                path = new Path(v, 0);
            } else{
                v.weightedCostFromStartVertex = INFINITY;
                path = new Path(v, INFINITY);
            }
            paths.add(path);
        }
        visit(paths);
    ...
    //helper method for dijkstra that visits all the paths
    //and adds them to the priority queue
    //@param passes the priority queue
    private static void visit(PriorityQueue<Path> queue){
        //while there are unvisited vertices,
        //let the current vertex bc the lowest cost,
        ...
        while (!queue.isEmpty()) {
            ...
            if (currentVertex.scratch == 0) {
                ...
                //update the cost of the destination if the sum is less than the
                //cost of the destination vertex
                //and set the previous of the destination vertex to the current
                for (Edge e : currentVertex.adjacent) {
                    Vertex neighbor = e.dest;
                    if (currentVertex.weightedCostFromStartVertex + e.cost <
                            neighbor.weightedCostFromStartVertex) {
                        Path path = new Path(neighbor, currentPath.weightedCost() + e.cost);
                        neighbor.weightedCostFromStartVertex =
                                currentVertex.weightedCostFromStartVertex + e.cost;
                        neighbor.numEdgesFromStartVertex =
                                currentVertex.numEdgesFromStartVertex + 1;
    ...
    public void findAllPaths(boolean weighted) {
        ...
        allPathsFound = true;
        longest = new Path();
        //if weighted is true, uses dijkstra's algorithm
        //otherwise uses the unweighted shortest path algorithm
        for (String name : vertices.keySet()) {
            Vertex current = vertices.get(name);
            current.clearPathInfo();
            clearAll();
            if(weighted){
                dijkstra(name);
            } else{
                findUnweightedShortestPath(name);
            }
            for (Vertex v : vertices.values()) {
                if (!v.equals(current) && v.weightedCostFromStartVertex < INFINITY) {
                    current.numVertexConnected++;
                    current.totalUnweightedPathLength += v.numEdgesFromStartVertex;
                    current.totalWeightedPathLength += v.weightedCostFromStartVertex;
                    //if weighted is true, use the weighted cost to compare
                    //otherwise, use the number of edges to compare (unweighted cost)
                    if (weighted){
                        if(v.weightedCostFromStartVertex > longest.weightedCostOfPath){
                            longest = getPath(v.name);
                        }
                    } else{
                        if(v.numEdgesFromStartVertex > longest.weightedCostOfPath){
                            longest = getPath(v.name);
                        }
                    }
                }
